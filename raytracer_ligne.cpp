...
void render(const std::vector<Sphere> &spheres)
{
    ...
    int rank, nbp, i_row_cur = 0, i_row_recu = 0;
    ...
    if ( rank == 0 ) {
        for ( int p = 1; p < nbp; ++p ) {
            MPI_Send(&i_row_cur, 1, MPI_INT, p, 101, MPI_COMM_WORLD);
            ++i_row_cur;
        }
        while (i_row_cur < H) {     
        std::vector<float> row_pixels(3*W);       
            MPI_Recv(row_pixels.data(), 3*W, MPI_FLOAT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD,&status);
            i_row_recu = status.MPI_TAG;
            for ( int j = 0; j < 3*W; ++j )
                pixels[3*W*i_row_recu+j] = row_pixels[j];
            MPI_Send(&i_row_cur, 1, MPI_INT, status.MPI_SOURCE, 101, MPI_COMM_WORLD);
            ++i_row_cur ;
        }
        int p = 1;
        while (p < nbp )
    ...
    else
    {
        ...
        std::vector<float> row_pixel(3*W);
        //
        int i_row_cur = 0;
        do {
            MPI_Recv(&i_row_cur, 1, MPI_INT, 0, 101, MPI_COMM_WORLD, &status );
            if ( i_row_cur < H )
            {
                  unsigned int y = i_row_cur;
                  ...
                  for (unsigned x = 0; x < W; ++x) {
                        ...
                        row_pixel[3*x]=image[W*y+x].x;
                        row_pixel[3*x+1]=image[W*y+x].y;
                        row_pixel[3*x+2]=image[W*y+x].z;
                  }
                  MPI_Send(row_pixel.data(), 3*W, MPI_FLOAT, 0, i_row_cur, MPI_COMM_WORLD);
            }
        } while (i_row_cur < H);
